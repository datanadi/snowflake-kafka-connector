...
public class DirectTopicPartitionChannel implements TopicPartitionChannel {
  ...
  @Override
  public InsertValidationResponse insertRecord(SinkRecord kafkaSinkRecord, boolean isFirstRowPerPartitionInBatch) {
    ...
    if (shouldInsertRecord(kafkaSinkRecord, currProcessedOffset)) {
      try {
        Map<String, Object> transformedData = transformDataForStreaming(kafkaSinkRecord, kafkaRecordErrorReporter);
        ...
        LOGGER.warn(
                String.format(
                        "[INSERT_RECORDS] Failure inserting rows for channel:%s", this.getChannelNameFormatV1()),
                ex);
      }
    }
  ...
  private boolean shouldInsertRecord(SinkRecord kafkaSinkRecord, long currentProcessedOffset) {
    if (currentProcessedOffset == NO_OFFSET_TOKEN_REGISTERED_IN_SNOWFLAKE) {
      LOGGER.debug(
              "Insert record because there is no currentProcessedOffset for channel:{}",
              this.getChannelNameFormatV1());
      return true;
    }
    ...
    if (kafkaSinkRecord.kafkaOffset() > currentProcessedOffset) {
      LOGGER.debug(
              "Insert record because kafkaOffset {} > currentProcessedOffset {} for channel:{}",
    ...
    LOGGER.debug(
            "Ignore adding kafkaOffset:{} in channel:{}. currentProcessedOffset:{}",
            kafkaSinkRecord.kafkaOffset(),
            this.getChannelNameFormatV1(),
  ...
  private Map<String, Object> transformDataForStreaming(SinkRecord kafkaSinkRecord, KafkaRecordErrorReporter kafkaRecordErrorReporter) {
    ...
    if (isRecordBroken(snowflakeRecord)) {
      ...
      try {
        Map<String, Object> tableRow = recordService.getProcessedRecordForStreamingIngest(snowflakeRecord);
   ...
   * InsertValidationResponse.InsertError} in form of response inside
   * {@link InsertValidationResponse}
   *
   * @param record
   * @param offset
   * @return {@link InsertValidationResponse} a response that wraps around InsertValidationResponse
   */
